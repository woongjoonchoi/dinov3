...
bash  docker/run_imagenet_eval_ddp_window_b1_1.sh  \
  --gpus all \
    --shm-size=120g \
  -v /projects3/datasets/imagenet/imagenet-1k:/datasets/imagenet:ro \
      -v ~/dinov3_pth:/dinov3_pth:ro \
  ...
  --device cuda \
  --distributed  \
  --model-kwargs '{"global_block_interval": 3}' 
...
docker build -f "${REPO_ROOT}/docker/imagenet_eval_ddp_window_b1_1.Dockerfile" -t "${IMAGE_NAME}" "${REPO_ROOT}"
