"""Extract DinoV3 Baseline1_1 backbone activations and save them per image/class.

This script mirrors ``tools/save_backbone_activations.py`` but targets the
``DinoVisionTransformerWindowBaseline1_1`` hybrid backbone. For each image in an
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=json.loads,
        default={},
        help="JSON object of keyword arguments forwarded to DinoVisionTransformerWindowBaseline1_1.",
    )
...
def _build_model(args: argparse.Namespace, device: torch.device) -> torch.nn.Module:
    ...
    backbone_state = _remap_window_block_keys_to_patch_only(backbone, _load_state_dict(args.backbone_checkpoint))
