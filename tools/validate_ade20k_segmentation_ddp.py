...
def _build_config(args: argparse.Namespace) -> SegmentationConfig:
    ...
    overrides = {
        "datasets": {
            "root": str(_resolve_dataset_root(args.dataset_root, split_dirname="validation")),
...
def _resolve_dataset_root(base_root: Path, *, split_dirname: str) -> Path:
    """Detect the ADE20K root that actually contains images/annotations.

    The user may point ``--dataset-root`` to the top-level ADE20K folder
    (containing ``images/`` and ``annotations/``) or to a parent directory that
    nests the actual data (e.g., ``ADE20K_2021_17_01``). This helper searches a
    couple of common layouts and returns the path expected by ``make_dataset``.
    """

    def _has_split(root: Path) -> bool:
        candidates = [
            root / "images" / "ADE" / split_dirname,
            root / "images" / split_dirname,
            root / split_dirname,
        ]
        return any(p.is_dir() for p in candidates)

    if not base_root.exists():
        raise FileNotFoundError(f"--dataset-root does not exist: {base_root}")

    base_root = base_root.resolve()
    if _has_split(base_root):
        return base_root

    for immediate in base_root.iterdir():
        if immediate.is_dir() and _has_split(immediate):
            logger.info("Detected ADE20K dataset root at %s", immediate)
            return immediate

    for images_dir in base_root.glob("**/images"):
        candidate_root = images_dir.parent
        if _has_split(candidate_root):
            logger.info("Detected ADE20K dataset root at %s via nested images/", candidate_root)
            return candidate_root

    raise FileNotFoundError(
        "Could not locate ADE20K images under %s; expected to find images/%s."
        % (base_root, split_dirname)
    )
