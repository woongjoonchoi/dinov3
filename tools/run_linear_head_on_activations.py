...
obtain logits and accuracy without re-running the backbone.  Use ``torchrun``
with ``--distributed`` to shard activation loading across multiple GPUs.
"""
...
def _init_distributed() -> tuple[torch.device, int, int]:
...
def _evaluate_head(
    ...
    collect_logits = save_logits is not None and (not distributed or rank == 0)
    logits_buffer = []
    ...
    for batch in loader:
        ...
        if save_logits is not None and distributed:
            gathered: list[Optional[torch.Tensor]]
            if rank == 0:
                gathered = [None for _ in range(dist.get_world_size())]
                dist.gather_object(logits_cpu, gathered, dst=0)
                logits_buffer.extend(gathered)
            else:
                dist.gather_object(logits_cpu, dst=0)
        elif collect_logits:
            logits_buffer.append(logits_cpu)
    ...
    if save_logits is not None and collect_logits:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pin-memory",
        action="store_true",
        help="Pin dataloader memory for faster host->device copies.",
    )
    parser.add_argument(
        "--save-logits",
        type=pathlib.Path,
...
def main() -> None:
    ...
    if args.distributed:
        ...
        linear_head = torch.nn.parallel.DistributedDataParallel(
            linear_head, device_ids=device_ids
        )
