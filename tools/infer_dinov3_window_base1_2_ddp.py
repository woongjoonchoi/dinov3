"""Run ImageNet-style inference with the Baseline1_2 hybrid DinoV3 backbone using DDP.

This script mirrors :mod:`tools.eval_imagenet_accuracy_ddp` but targets the
``DinoVisionTransformerWindowLastGlobal`` backbone (Baseline1_2) where the tail
Transformer blocks run full global attention while earlier layers use window
attention.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        required=True,
        help="Checkpoint containing the DinoVisionTransformerWindowLastGlobal weights.",
    )
    ...
    parser.add_argument(
        ...
        type=json.loads,
        default={},
        help="JSON object of keyword arguments forwarded to DinoVisionTransformerWindowLastGlobal (e.g. '{\"num_last_global_blocks\": 4}')",
    )
